...
def score_indicator(key: str, current: float | None, previous: float | None) -> int:
    ...
    if key == "IntRate":
        ...
        if delta > 0.1:
            ...
            return 1 if current >= 4.0 else 0
        elif delta > -0.1:
            return -1
        else:
            return -2
...
def compute_indicator(key: str, cfg: dict, obs: list[tuple[str, float]]) -> dict | None:
    ...
    trend = (
        ("opp" if current > previous else "ned" if current < previous else "flat") if previous is not None else "ukjent"
    )
...
def consensus_multiplier(cat_scores: dict) -> float:
    ...
    THRESHOLD = 0.35
    infl_dir = (
        1 if cat_scores.get("inflation", 0) > THRESHOLD else -1 if cat_scores.get("inflation", 0) < -THRESHOLD else 0
    )
    jobs_dir = 1 if cat_scores.get("jobs", 0) > THRESHOLD else -1 if cat_scores.get("jobs", 0) < -THRESHOLD else 0
    growth_dir = (
        1
        if cat_scores.get("econ_growth", 0) > THRESHOLD
        else -1
        if cat_scores.get("econ_growth", 0) < -THRESHOLD
        else 0
    )
